...
def attention(x: jax.Array, layer_weights: LayerWeights, model_params, pos: int, layer_idx: int, freqs_cis: jax.Array, kvcache: KVCache, attn_mask: Optional[jax.Array] = None) -> Tuple[jax.Array, KVCache]:
  bsz, _, _ = x.shape
  ...
  keys, values, kvcache = kvcache.update(xk, xv, layer_idx, pos, n_rep)
