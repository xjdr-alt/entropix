...
def calculate_metrics(
  ...
  agreement = jnp.mean(
    jnp.abs(attention_probs - mean_attention[:, None, :]), axis=(1, 2)
  )
...
# implements EntropySampler[SamplerConfig, None]
def sample(
  ...
  *,
  ...
  key=jax.random.PRNGKey(1337),
  # the None in the tuple satisfies the EntropySampler protocol.
  # the baseline sampler is currently stateless but this can change later
  ...
  if ent < cfg.low_ent_thresh and vent < cfg.low_vent_thresh:
    return jnp.argmax(logits[:, -1], axis=-1, keepdims=True).astype(jnp.int32), None
  ...
  # High Entropy, Low Varentropy: "treading carefully, asking clarifying questions"
    ...
    if not jnp.isin(gen_tokens[:, -1], clarifying_question_token).any():
      return jnp.array([[clarifying_question_token]]), None
    else:
      # If we've just asked a question, sample with slightly higher temperature
      temp_adj = (
        cfg.helv_attn_ent_offset + cfg.helv_attn_ent_coef * attn_ent
  ...
  # Low Entropy, High Varentropy: "exploring forks in the path"
    ...
    temp_adj = (
      cfg.lehv_interaction_strength_offset
      + cfg.lehv_interaction_strength_coef * interaction_strength
    )  # Increase temperature based on interaction strength
    top_k_adj = max(
      5, int(cfg.top_k * (1 + 0.5 * (1 - agreement)))
  ...
  # High Entropy, High Varentropy: "resampling in the mist"
    ...
    temp_adj = (
      cfg.hehv_attn_vent_offset + cfg.hehv_attn_vent_coef * attn_vent
    )  # Increase temperature based on attention varentropy
    top_p_adj = max(
      0.5, cfg.top_p - cfg.hehv_attn_ent_coef * attn_ent
  ...
  # Middle ground: use adaptive sampling
    ...
    temperature = cfg.temp * (
      ...
      - cfg.ada_temp_agree * metrics["agreement"]
    )
    top_p = jnp.clip(
      cfg.top_p * (1 + cfg.ada_top_p * metrics["attn_varentropy"]), 0.1, 1.0
    )
    top_k = int(
      jnp.clip(
        jnp.round(
          ...
          * (
            1
            + cfg.ada_top_k_int * metrics["interaction_strength"].item()
