...
from typing import List, Optional, Literal, AsyncGenerator, Tuple
import time
import logging
import json
import uuid
...
class ModelManager:
    ...
    def format_prompt(self, messages: List[Message]) -> str:
        formatted_messages = []
        for msg in messages:
            formatted_messages.append(f"<|start_header_id|>{msg.role}<|end_header_id|>\n{msg.content}<|eot_id|>")
        return "<|begin_of_text|>" + "".join(formatted_messages)

    async def generate_response(self, prompt: str, max_tokens: int) -> AsyncGenerator[Tuple[str, List[int]], None]:
...
async def stream_response(request: ChatCompletionRequest) -> AsyncGenerator[str, None]:
    ...
    prompt = model_manager.format_prompt(request.messages)
    accumulated_text = ""

    try:
        async for token_tuple in model_manager.generate_response(prompt, request.max_tokens):
