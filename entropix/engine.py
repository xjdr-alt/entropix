...
class EntropixEngine:
  ...
  def precompute_freqs_cis(self, dim: int, end: int, theta: float = 500000.0, use_scaled: bool = False, dtype: jnp.dtype = jnp.float32) -> jax.Array:
  ...
  def generate(
    ...
    result = ResultTokens(
      data=jnp.concatenate((new_token, jnp.ones_like(new_token, dtype=jnp.bool_), decode_state["generated_tokens"]), axis=1),
      # Tokens are shape [batch, speculations], so when we concatenate
      # tokens, validity and length along their index 1 dimension then they
      # occupy 0:speculations.
      tokens_idx=(0, 1),
